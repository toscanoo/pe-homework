...
# %%
# à vous
df3=df2.resample('1W').mean()
df4=df3[['High','Low']]
print(df4.count())
print(df2.count())
df4.plot()
#il devrait y avoir autant de valeurs que de semaines pendant la totalité du temps
#que couvre la dataframe après le resample, or la dataframe commence en
#2018-07-02 et finit le 2019-12-31 soit L=547 jours et 547/7=~78.1 (valeur réelle 79) la différence  s'explique
#a cause de la derniere semaine qui est incomplete mais crée qd meme une bin apres le resample
#ainsi le nombre de valeur est exactement (L//7) +1 (ou // est la division euclidienne)
...
# %%
# à vous
df5=df2.rolling(window=pd.Timedelta(365, 'D'),center=True).mean()
df5.plot()
#il y a autant d'entrées que de valeur dans la dataframe, le rolling n'as fait que lisser la courbe en moyennant chaque valeur
